...
check_token() {
  echo -e "                                                       "
  echo -e "${BLUE}[+] =============================================== [+]${NC}"
  echo -e "${BLUE}[+]               KUNCI BY HarleyHosting             [+]${NC}"
  echo -e "${BLUE}[+] =============================================== [+]${NC}"
  echo -e "                                                       "
  echo -e "${YELLOW}MASUKAN AKSES TOKEN :${NC}"
  read -r USER_TOKEN

  if [ "$USER_TOKEN" = "HarleyHosting" ]; then
    echo -e "${GREEN}AKSES BERHASIL${NC}}"
  else
    echo -e "${GREEN}Buy dulu wkww Ke HarleyHosting Telegram @HarleyHosting${NC}"
    exit 1
  fi
  clear
}
...
install_theme() {
  while true; do
    ...
    case "$SELECT_THEME" in
      1)
        THEME_URL=$(echo -e "https://github.com/harleyfebrykurniawan11/HarleyHost/raw/main/C2.zip")
        break
        ;;
      2)
...
# Unduh file tema
wget -O /root/C2.zip https://github.com/harleyfebrykurniawan11/HarleyHost/raw/main/C2.zip
